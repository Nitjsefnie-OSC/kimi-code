/**
 * `tools` domain (L7) — `MonitorTask`, the `AgentTask` adapter behind the
 * `Monitor` tool.
 *
 * Watches a spawned process's stdout: every raw chunk is teed to the task
 * sink (so TaskOutput sees it), complete lines are batched (`batchMs`
 * debounce) and handed to the injected `emit` for notification delivery, and
 * a sliding-window volume cap (`maxLinesPerWindow` per `volumeWindowMs`)
 * force-stops noisy commands with a warning. A trailing unterminated line is
 * flushed when the process ends. Ported from v1's `MonitorBackgroundTask`
 * (`packages/agent-core/src/agent/background/monitor-task.ts`).
 */
